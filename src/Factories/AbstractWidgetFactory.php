...
namespace Widgets\Factories;

use Widgets\AbstractWidget;
use Widgets\Contracts\ApplicationWrapperContract;
use Widgets\Misc\EncryptException;
use Widgets\Misc\InvalidWidgetClassException;
use Widgets\Misc\ViewExpressionTrait;
...
abstract class AbstractWidgetFactory
{
    ...
    protected function instantiateWidget(array $params = [])
    {
        ...
        if (!is_subclass_of($widgetClass, 'Widgets\AbstractWidget')) {
            throw new InvalidWidgetClassException('Class "'.$widgetClass.'" must extend "Widgets\AbstractWidget" class');
        }
